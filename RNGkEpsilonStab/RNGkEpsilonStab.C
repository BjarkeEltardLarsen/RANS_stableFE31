...
defineTypeNameAndDebug(RNGkEpsilonStab, 0);
addToRunTimeSelectionTable(RASModel, RNGkEpsilonStab, dictionary);

// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

RNGkEpsilonStab::RNGkEpsilonStab
(
    ...
    )
{
 ...
 nut_ = Cmu_*sqr(k_)/max(epsilon_ + epsilonSmall_,1.2603*lambda2_*C2_/C1_*2*magSqr(symm(fvc::grad(U_)))/(2*magSqr(skew(fvc::grad(U_)))+pOmegaSmall_)*epsilon_); //nut as suggested by Larsen and Fuhrman (2018)
...
tmp<volSymmTensorField> RNGkEpsilonStab::R() const
{
...
tmp<volSymmTensorField> RNGkEpsilonStab::devReff() const
{
    return tmp<volSymmTensorField>
    (
        new volSymmTensorField
...
tmp<fvVectorMatrix> RNGkEpsilonStab::divDevReff(volVectorField& U) const
{
    return
    (
...
// Calculate eddy viscosity using the stabilizing approach
 ...
 volScalarField epsilonTilde = max(epsilon_ + epsilonSmall_,1.2603*lambda2_*C2_/C1_*p0*epsilon_/(pOmega+pOmegaSmall_));

 nut_ = Cmu_*sqr(k_)/epsilonTilde; //nut calculated  suggested by Larsen and Fuhrman (2018)
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * //
