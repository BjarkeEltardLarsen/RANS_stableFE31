...
{
  nut_ = Cmu_*sqr(k_)/max(epsilon_ + epsilonSmall_,lambda2_*C2_/C1_*2*magSqr(symm(fvc::grad(U_)))/(2*magSqr(skew(fvc::grad(U_)))+pOmegaSmall_)*epsilon_); //nut  as suggested by Larsen and Fuhrman (2018)
