...
{
  nut_ = k_/max(omega_ + omegaSmall_,lambda2_*beta_/(Cmu_*alpha_)*2*magSqr(symm(fvc::grad(U_)))/(2*magSqr(skew(fvc::grad(U_)))+pOmegaSmall_)*omega_); //nut as suggested by Larsen and Fuhrman 2018
