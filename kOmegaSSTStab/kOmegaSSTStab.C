    ...
    )
{
    ...
    nut_ = a1_*k_/max(a1_*lambda2_*beta1_/(betaStar_*gamma1_)*2*magSqr(symm(fvc::grad(U_)))/(2*magSqr(skew(fvc::grad(U_)))+pOmegaSmall_)*omega_,max(a1_*omega_, F2()*sqrt(2.0)*mag(symm(fvc::grad(U_))))); //nut as suggested by Larsen and Fuhrman 2018
